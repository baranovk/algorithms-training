namespace AlgorithmsTraining.Arrays
{
    ...
    Constraints:
      ...
      [4] The input is generated such that the answer is unique.
    ...
    public static class GasStation
    {
        ...
        {
            if (1 == gas.Length) { return gas[0] >= cost[0] ? 0 : -1; }

            for (int i = 0; i < gas.Length; i++)
            {
                for (int j = i, length = 0, tank = 0; j < gas.Length; j = (++j % gas.Length))
                {
                    tank += gas[j] - cost[j];

                    if (tank < 0)
                    {
                        i = j < i ? gas.Length - 1 : j;
                        break;
                    }
                    else if (++length == gas.Length)
                    {
                        return i;
                    }
                }
        ...
        public static int CanCompleteCircuit_0(int[] gas, int[] cost)
        {
            if (1 == gas.Length) { return gas[0] >= cost[0] ? 0 : -1; }
