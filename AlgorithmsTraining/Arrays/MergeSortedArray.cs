 ...
 * 88. Merge Sorted Array
  ...
  nums1. To accommodate this, nums1 has a length of m + n, where the first m elements denote the elements that should be 
...
public static class MergeSortedArray
{
    ...
    {
        var currentMergeIndex = m + n - 1;
        var nums1Index = m - 1;
        var nums2Index = n - 1;

        while (currentMergeIndex >= 0)
        {
            nums1[currentMergeIndex--] = 
                (nums1Index >= 0 ? nums1[nums1Index] : int.MinValue) >= (nums2Index >= 0 ? nums2[nums2Index] : int.MinValue)
                ? nums1[nums1Index--] 
                : nums2[nums2Index--];
        }
    }
