namespace AlgorithmsTraining.Lists
{
    ...
    public static class CopyListWithRandomPointer
    {
        ...
        public static Node CopyRandomList(Node head)
        {
            ...
            prev = clone = Clone(head, dict);

            while ((head = head.next) != null)
            {
                var current = Clone(head, dict);
                prev.next = current;
                prev = current;
            }
        ...
        private static Node Clone(Node node, Dictionary<Node, Node> dict)
        {
            if (!dict.TryGetValue(node, out var existing))
            {
                dict.Add(node, new Node(node.val));
                existing = dict[node];
                existing.random = null == node.random ? null : Clone(node.random, dict);
            }
